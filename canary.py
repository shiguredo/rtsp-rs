...
# cargo update shiguredo_rtsp を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_rtsp'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_rtsp"], check=True)
        print("cargo update shiguredo_rtsp executed")
